...
class backend:    
    def user_table(self):
        with sqlite3.connect("data.db") as connection:
            ...
            cursor.execute("""CREATE TABLE IF NOT EXISTS user (
                           ...
                           windows_hello_enrolled INTEGER DEFAULT 0
                           )""")
            connection.commit()
    ...
    def create_user(self, account_holder, password, balance_on_card, windows_hello_enrolled):
        with sqlite3.connect("data.db") as connection:
            cursor = connection.cursor()
            cursor.execute("""INSERT INTO user VALUES(NULL, ?,?,?,?)""", (account_holder, password, balance_on_card, windows_hello_enrolled))
            connection.commit()
    ...
    def get_user_id(self, account_holder):
                with sqlite3.connect("data.db") as connection:
                    ...
                    # Fetch user_id for the given holder
                    cursor.execute("SELECT user_id FROM user WHERE account_holder=?", (account_holder,))
    ...
    def log_transaction(self, user_id, amount, transaction_type):
        ...
        # Insert the transaction record into the transactions table
            cursor.execute("INSERT INTO transactions VALUES (?, ?, ?)", (user_id, amount, transaction_type))
        
            connection.commit()
    
    def update_balance(self, amount, account_balance, account_holder):
        with sqlite3.connect("data.db") as connection:
            cursor = connection.cursor()
            cursor.execute("UPDATE user SET account_balance=account_balance + ? WHERE account_holder=?", (amount, account_balance, account_holder))
