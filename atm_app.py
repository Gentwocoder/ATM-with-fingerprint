...
class Signin(tk.Frame):
    def __init__(self, parent, controller):
        ...
        async def login_user(account_holder, password):
            # Fetch user details based on the card number
            cursor.execute("SELECT password, windows_hello_enrolled FROM user WHERE account_holder = ?", (account_holder,))
            result = cursor.fetchone()


            if result:
                stored_password, fingerprint_enrolled = result
                
                if bcrypt.checkpw(password.encode("utf-8"), stored_password):
                    if fingerprint_enrolled == 1:
                        # if await authenticate_user():
                        if authenticate_with_touch():
                            # Store the logged-in user details in a global dictionary
                            current_user['first_name'] = account_holder.split(" ")[0]
                            current_user['full_name'] = account_holder
        ...
        async def signin():
            get_fullname = fullname_entry.get()
            get_password = password_entry.get()
            if len(get_password) != 0 and len(get_fullname) != 0:
                if await login_user(get_fullname.lower(), get_password):
        ...
        load_img = Image.open("images/logon.png")
        ...
        def on_leave(e):
            card = fullname_entry.get()
            if card == "":
                fullname_entry.insert(0, "Enter full name")
            
        fullname_entry = tk.Entry(frame, width=25, fg="black", border=0, bg="white", font=("Microsoft YaHei UI Light", 11))
        fullname_entry.place(x=30, y=80)
        fullname_entry.insert(0, "Enter full name")

        fullname_entry.bind("<FocusIn>", on_enter)
        fullname_entry.bind("<FocusOut>", on_leave)
        ...
        tk.Button(frame, width=30, pady=7, text="Enter", bg="white", fg="black", border=0, command=lambda: asyncio.run(signin())).place(x=35, y=204)
        label = tk.Label(frame, text="Don't have an account", fg="black", bg="white", font=("Microsoft YaHei UI Light", 9))
        label.place(x=75, y=270)

        sign_up = tk.Button(frame, width=9, text="Sign up", border=0, bg="white", cursor="hand2", fg="blue", command=lambda: controller.show_frame(Signup))
...
class Signup(tk.Frame):
    def __init__(self, parent, controller):
        ...
        async def register():
            ...
            fullname = get_firstname.lower() + " " + get_lastname.lower()
            ...
            if len(get_firstname) != 0 and len(get_lastname) != 0 and len(get_password) !=0 and len(get_password2) !=0:         
                if get_password != get_password2:
                    ...
                    ch.create_user(self, fullname, hashed_password, 0.0, fingerprint_enrolled)
        ...
        ## window image
        load_img = Image.open("images/login.png")
        img = ImageTk.PhotoImage(load_img)
        ...
        heading = tk.Label(frame, text="sign up to atm", fg="#57a1f8", bg="white", font=("Microsoft YaHei UI Light", 23, "bold"))
        ...
        last_name.bind("<FocusIn>", on_enter_lastname)
        ...
        tk.Button(frame, width=30, pady=7, text="Sign up", bg="#57a1f8", fg="black", border=0, command=lambda: asyncio.run(register())).place(x=32, y=320)
        label = tk.Label(frame, text="I have an account", fg="black", bg="white", font=("Microsoft YaHei UI Light", 9))
        label.place(x=90, y=370)

        signin = tk.Button(frame, width=6, text="Sign in", border=0, bg="white", cursor="hand2", fg="#57a1f8", command=lambda: controller.show_frame(Signin))
        signin.place(x=200, y=365)
...
class Mainpage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def update_password():
            ...
            window.geometry("300x300+300+200")
            window.resizable(False, False)
            old_password_entry = tk.Entry(window, width=25, fg="black", border=0, bg="white", font=("Micrfosoft YaHei UI Light", 11))
            new_password_entry = tk.Entry(window, width=25, fg="black", border=0, bg="white", font=("Micrfosoft YaHei UI Light", 11))
            confirm_password_entry = tk.Entry(window, width=25, fg="black", border=0, bg="white", font=("Micrfosoft YaHei UI Light", 11))
            old_password_entry.place(x=30, y=50)
            new_password_entry.place(x=30, y=100)
            confirm_password_entry.place(x=30, y=150)
            ...
            tk.Button(window, text="Change Password", command=change_password, bg="#57a1f8", font=("Microsoft YaHei UI Light", 9, "bold"), width=24).place(x=30, y=220)
            window.mainloop()


        label = tk.Label(self, text="Welcome", border=0, bg="white", fg="#57a1f8", font=("Microsoft YaHei UI Light", 23, "bold"))
        label.place(x=350, y=15)

        label2 = tk.Label(self, text="What would you like to do?", border=0, bg="white",fg="#57a1f8", font=("Microsoft YaHei UI Light", 23, "bold"))
        label2.place(x=260, y=50)

        label1 = tk.Label(self, border=0, bg="white",fg="#57a1f8", font=("Microsoft YaHei UI Light", 23, "bold"))
        label1.place(x=290, y=90)

        btn1 = tk.Button(self, width=30, pady=5, text="Deposit", bg="#57a1f8", fg="black", border=0, command=deposit)
        btn2 = tk.Button(self, width=30, pady=5, text="Withdraw", bg="#57a1f8", fg="black", border=0, command=withdraw)
        btn3 = tk.Button(self, width=30, pady=5, text="Check Balance", bg="#57a1f8", fg="black", border=0, command=check_balance)
        btn4 = tk.Button(self, width=30, pady=5, text="Change password", bg="#57a1f8", fg="black", border=0, command=update_password)
